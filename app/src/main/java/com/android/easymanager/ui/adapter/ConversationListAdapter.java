...
import com.android.easymanager.ui.widget.SwipeLayoutConv;
import com.android.easymanager.ui.widget.ViewHolder;
import com.android.easymanager.utils.SharePreferenceManager;
import com.android.easymanager.utils.SortConvList;
import com.android.easymanager.utils.SortTopConvList;
import com.android.easymanager.utils.ThreadUtil;
import com.android.easymanager.utils.TimeFormat;
...
public class ConversationListAdapter extends BaseAdapter {
    ...
    public ConversationListAdapter(Context context, List<Conversation> data, ConversationListView listView) {
    ...
    public void setToTop(Conversation conv) {
        ...
        for (Conversation conversation : mDatas) {
            if (conv.getId().equals(conversation.getId())) {
                ...
                if (!TextUtils.isEmpty(conv.getExtra())) {
                    ...
                    mUIHandler.sendEmptyMessageAtTime(REFRESH_CONVERSATION_LIST, 200);
                    return;
                }
            }
        }
        if (mDatas.size() == 0) {
            ...
            for (int j = mDatas.size(); j > SharePreferenceManager.getCancelTopSize(); j--) {
                if (conv.getLatestMessage() != null && mDatas.get(j - 1).getLatestMessage() != null) {
                    if (conv.getLatestMessage().getCreateTime() > mDatas.get(j - 1).getLatestMessage().getCreateTime()) {
                        newCount = j - 1;
                    } else {
                        newCount = j;
                        break;
    ...
    public void sortConvList() {
        ...
        mDatas.removeAll(forCurrent);
        if (topConv != null && topConv.size() > 0) {
            SortTopConvList sortTopConvList = new SortTopConvList();
            Collections.sort(topConv, sortTopConvList);
            for (Conversation conversation : topConv) {
                mDatas.add(i, conversation);
                i++;
            }
        }
        notifyDataSetChanged();
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convItem.getUnReadMsgCnt() > 0) {//如果对话中有未读信息
...
//禁止使用侧滑功能.如果想要使用就设置为true
    ...
    public static class UIHandler extends Handler {
        ...
        private WeakReference<ConversationListAdapter> mAdapter;

        public UIHandler(ConversationListAdapter conversationListAdapter) {
            mAdapter = new WeakReference<>(conversationListAdapter);
        }
